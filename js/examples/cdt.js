...
window.onload = function () {
  ...
  tool.onMouseUp = function(event) {
    ...
    if (total < 1) {
      ...
      var contour = path.segments.map( function (segment) {
        return new poly2tri.Point(segment.point.x, segment.point.y);
      })
      ...
      triangles.forEach( function (t) {
        ...
        for (var i=0; i<points.length; i++) {
          ...
          var from = new Point(points[i].x, points[i].y);
          var to = new Point(points[ni].x, points[ni].y);
